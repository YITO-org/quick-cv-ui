                ...
                <Grid container  columnGap={1} >
                    <Grid xs={12} sm={12} md={12} lg={5} xl={5}>
                            <Paper   sx={styles.detailes_box}>
                                <Typography sx={{mt : 1 , mb:1 }} textAlign='center' variant='h5' fontWeight='500' >Details</Typography>
                                <Box sx={{p : 2}}>
                                    {
                                        ['name' , 'designation' , 'dob' , 'phoneNumber' , 'email' , 'github' , 'linkdin'].map((e,index)=>(
                                            <Box key={index}>
                                                <InputLabel sx={styles.input_lable}>{e}</InputLabel>
                                                <TextField type="text" size="small" placeholder={e} name={e} value={details[e]}  onChange={change} sx={{marginBottom : 2 , width : '100%'}} />
                                            </Box>
                                        ))
                                    }
                                    </Box>
